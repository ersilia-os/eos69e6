...
def build_dgl_graph(u, v, dis, node_type: np.array, node_size: np.array):
  ...
  g.ndata["h"] = torch.FloatTensor(
    np.concatenate((node_type, node_size.reshape(-1, 1)), axis=1)
  )
  return g
...
def load_phar_file(file_path: Path):
  load_file_fn = {".posp": load_pp_file, ".edgep": load_ep_file}.get(
    file_path.suffix, None
  )

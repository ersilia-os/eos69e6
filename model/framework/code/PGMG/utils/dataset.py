...
class Tokenizer:
  ...
  SPECIAL_TOKENS += tuple([
    f"<t_{i}>" for i in range(len(SPECIAL_TOKENS), 32)
  ...
  def __init__(self, vocabs):
    ...
    vocabs = special_tokens + sorted(
      set(vocabs) - set(special_tokens), key=lambda x: (len(x), x)
    )
    self.vocabs = vocabs
...
class SemiSmilesDataset(Dataset):
  def __init__(
    ...
    smiles_list,
    tokenizer: Tokenizer,
    use_random_input_smiles=False,
    use_random_target_smiles=False,
    rsmiles=None,
    corrupt=True,
  ):
    """
    ...
    if rsmiles is None and (use_random_input_smiles or use_random_target_smiles):
      print(
        "WARNING: The result of rdkit.Chem.MolToSmiles(..., doRandom=True) is NOT reproducible "
  ...
  def __getitem__(self, item):
    ...
    csmiles = Chem.MolToSmiles(
      mol, isomericSmiles=False, canonical=True, doRandom=False
    )
    if self.rsmiles is not None:
      rsmiles = self.rsmiles[item]
    else:
      rsmiles = Chem.MolToSmiles(
        mol, isomericSmiles=False, canonical=False, doRandom=True
      )
    ...
    mapping[
      :, pp_graph.num_nodes() :
    ] = -100  # torch cross entropy loss ignores -100 by default
  ...
  def collate_fn(batch):
    ...
    corrupted_inputs, pp_graphs, mappings, target_seqs, *other_descriptors = list(
      zip(*batch)
    )

    corrupted_inputs = pad_sequence(
      corrupted_inputs, batch_first=True, padding_value=pad_token
    )
    input_mask = (corrupted_inputs == pad_token).bool()

    pp_graphs = dgl.batch(pp_graphs)

    mappings = pad_sequence(
      mappings, batch_first=True, padding_value=-100
    )  # torch cross entropy loss ignores -100 by default, but we do not use cross_entropy_loss acctually

  ...
  for idx in range(len(path_atom) - 1):
    ...
    for bond_type in nei_bond:
      if (
        len(
          list(
            set([bond_type[1], bond_type[2]]).intersection(set([bond_start, bond_end]))
          )
        )
        == 2
      ):
...
def smiles2ppgraph(smiles: str):
  ...
  for f in feats:
    ...
    pharmocophore_all.append(
      pharmocophore_
    )  # all pharmacophore features within a molecule
  ...
  for pharmocophore_all_i in range(len(mol_phco)):
    for pharmocophore_all_j in range(len(mol_phco)):
      if (
        mol_phco[pharmocophore_all_i][1] == mol_phco[pharmocophore_all_j][1]
        and mol_phco[pharmocophore_all_i][0] != mol_phco[pharmocophore_all_j][0]
      ):
        index_ = [
          min(mol_phco[pharmocophore_all_i][0], mol_phco[pharmocophore_all_j][0]),
  ...
  for mol_phco_candidate_single in mol_phco:
    if mol_phco_candidate_single not in unique_index:
      if type(mol_phco[0]) == list:
        ...
        unique_index.append([
          [mol_phco_candidate_single[0]],
          mol_phco_candidate_single[1],
        ])
  for unique_index_single in unique_index:
    if unique_index_single not in unique_index_filter:
      unique_index_filter.append(
        unique_index_single
      )  ## The following is the order of the pharmacophores by atomic number
  sort_index_list = []
  for (
    unique_index_filter_i
  ) in unique_index_filter:  ## Collect the mean of the participating elements
      ...
      if mol_phco_i_elment == mol_phco_j_elment:
        ...
        if max(len(mol_phco_i_elment), len(mol_phco_j_elment)) == 1:
          ...
          position_matrix[mol_phco_i, mol_phco_j] = (
            min_dist + max(len(mol_phco_i_elment), len(mol_phco_j_elment)) * 0.2
          )
      else:
...
if __name__ == "__main__":
  ...
  g_, smiles_code_res = smiles2ppgraph(
    smiles
  )  ##g_药效团；smiles_code_res：smiles编码（矩阵格式）

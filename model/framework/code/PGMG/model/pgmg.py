...
class PGMG(nn.Module):
  PARAM_SET = {
    "max_len",  # max length of generated molecules
  ...
  def __init__(self, params, tokenizer):
    ...
    self.pp_encoder = GGCNEncoderBlock(
      hidden_dim,
      hidden_dim,
      n_layers=params["pp_encoder_n_layer"],
      dropout=0,
      readout_pooling="max",
      batch_norm=True,
      residual=True,
    ...
    self.encoder = TransformerEncoder(
      hidden_dim, ff_dim, num_head=n_head, num_layer=n_layers
    )
    ...
    self.mapping_transform_v = nn.Sequential(
      nn.Linear(hidden_dim, hidden_dim), nn.PReLU(), nn.Linear(hidden_dim, hidden_dim)
    )
  ...
  def calculate_z(self, inputs, input_mask, vvs, pp_mask):
    ...
    ppxt = self.encoder(
      ppxt, encoder_full_mask
    )  # (s b f), input masks need not transpose
  ...
  def resample(self, z):
    ...
    kl_loss = (
      -0.5 * torch.sum(1 + z_log_var - z_mean.pow(2) - z_log_var.exp()) / batch_size
    )
  ...
  def process_p(self, pp_graphs):
    ...
    vv = pad_sequence(
      torch.split(v, pp_graphs.batch_num_nodes().tolist()),
      batch_first=False,
      padding_value=-999,
    )
  ...
  def forward(self, inputs, input_mask, pp_graphs, targets):
    ...
    output = (
      self
      .decoder(target_embed, zzz, x_mask=target_mask, mem_padding_mask=encoder_mask)
      .permute(1, 0, 2)
      .contiguous()
    )
  ...
  def _generate(self, zzz, encoder_mask, random_sample, return_score=False):
    ...
    token = torch.full(
      (batch_size, self.max_len), self.pad_value, dtype=torch.long, device=device
    )
    ...
    for t in range(1, self.max_len):
      one = self.decoder.forward_one(
        text_embed, zzz, incremental_state, mem_padding_mask=encoder_mask
      )
      one = one.squeeze(0)
  ...
  def ag_forward(self, inputs, input_mask, pp_graphs, random_sample=False):
    ...
    predict, scores = self._generate(
      zzz, random_sample=random_sample, return_score=True
    )
  ...
  def generate(self, pp_graphs, random_sample=False, return_z=False):
    ...
    predict = self._generate(
      zzz, encoder_mask, random_sample=random_sample, return_score=False
    )

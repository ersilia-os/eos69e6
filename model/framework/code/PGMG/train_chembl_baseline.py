...
class CFG:
  ...
  gen_size = (
    2048  # number of pharmacophore graphs used to generate molecules during training
  )
...
def train_fn(train_loader, model, optimizer, epoch, scheduler, beta=1, scaler=None):
  ...
  for step, batch_data in tqdm(enumerate(train_loader), disable=disable_tqdm, total=N):
    ...
    inputs, input_mask, pp_graphs, mappings, targets, *others = [
      i.to("cuda:0") for i in batch_data
    ]
    ...
    if CFG.fp16:
      with amp.autocast():
        prediction_scores, mapping_scores, lm_loss, kl_loss = model(
          inputs, input_mask, pp_graphs, targets
        )
    else:
      prediction_scores, mapping_scores, lm_loss, kl_loss = model(
        inputs, input_mask, pp_graphs, targets
    ...
    xx = pad_sequence(
      torch.split(pp_graphs.ndata["type"], tuple(pp_graphs.batch_num_nodes().cpu())),
      batch_first=True,
    )
    x[:, : xx.shape[1], :] = xx
    ...
    mapping_loss_weight = (mappings == 1) * (
      8 / (0.001 + (mappings == 1).sum(1))
    ).unsqueeze(1)  # balance pos/neg samples
    mapping_loss_weight += (mappings != -100) * sample_weight.unsqueeze(
      1
    )  # balance rare pharmacophore types

    mapping_loss = F.binary_cross_entropy(
      mapping_scores, mappings, weight=mapping_loss_weight
    )
...
def valid_fn(valid_loader, model, epoch, beta=1, scaler=None):
  ...
  for step, batch_data in tqdm(enumerate(valid_loader), disable=disable_tqdm, total=N):
    ...
    map_acc = (
      (mapping_scores[mappings == 0] < 0.5).sum()
      + (mapping_scores[mappings == 1] >= 0.5).sum()
    ) / (mappings != -100).sum()
    ...
    if step % CFG.print_freq == 0 or step == (len(valid_loader) - 1):
      print(
        f"VALID Epoch: [{epoch + 1}][{step}/{len(valid_loader)}] "
        f"Data {data_time.val:.3f} ({data_time.avg:.3f}) "
...
def test_generate(valid_loader, model, epoch, random_sampling=False):
  ...
  for step, batch_data in tqdm(
    enumerate(valid_loader), disable=disable_tqdm, total=len(valid_loader)
  ):
    inputs, input_mask, pp_graphs, mappings, targets, *others = [
      i.to("cuda:0") for i in batch_data
    ]
  ...
  match_score = get_match_score(
    pp_graph_list, res, n_workers=CFG.num_workers, timeout=10
  )
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--model_type",
    choices=["rs_mapping", "cs_mapping", "non_vae", "remove_pp_dis"],
    default="rs_mapping",
  )
  ...
  gen_smiles = (
    list(
      np.random.RandomState(CFG.seed).choice(valid_smiles, CFG.gen_size, replace=False)
    )
    * CFG.gen_repeat
  )
  ...
  use_random_input_smiles = (
    MODEL_SETTINGS[args.model_type].setdefault("in", "rs") == "rs"
  )
  use_random_target_smiles = (
    MODEL_SETTINGS[args.model_type].setdefault("out", "rs") == "rs"
  )

  train_dataset = SemiSmilesDataset(
    train_smiles, tokenizer, use_random_input_smiles, use_random_target_smiles
  )
  valid_dataset = SemiSmilesDataset(
    valid_smiles, tokenizer, use_random_input_smiles, use_random_target_smiles
  )
  gen_dataset = SemiSmilesDataset(
    gen_smiles, tokenizer, use_random_input_smiles, use_random_target_smiles
  )
  ...
  optimizer = AdamW(
    model.parameters(), lr=CFG.init_lr, weight_decay=CFG.weight_decay, amsgrad=False
  )
  ...
  for epoch in range(CFG.init_epoch, CFG.epochs):
    ...
    # train
    avg_loss = train_fn(
      train_loader, model, optimizer, epoch, scheduler, beta, scaler=scaler
    )
    ...
    print(
      f"Epoch {epoch + 1} - beta {beta} avg_train_loss: {avg_loss:.4f} avg_valid_loss: {val_loss:.4f}  "
      f"time: {elapsed:.0f}s"
    )
